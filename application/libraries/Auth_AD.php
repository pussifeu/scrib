...
class Auth_AD
{
    ...
    function login($username, $password)
    {
        ...
        if (strlen($username) > 0 && strlen($password) > 0) {
            ...
            if ($continue) {
                ...
                if (!$entries = $this->search_ad($username, array('cn', 'dn', 'uid', 'mail'))) {
                    $continue = false;
                }
            }
    ...
    function getUserInfos($username)
    {
        ...
        if (strlen($username) > 0) {
            ...
            if ($continue) {
                // search for the user in the AD
                if (!$entries = $this->search_multiple_ad($username, array('uid', 'sn', 'givenname', 'mail', 'departmentNumber', 'gardianHierarchie'))) {
                    $continue = false;
                }
            }

            if ($continue) {
                // $entries n'est pas vide
                $return = array();

                for ($i = 0; $i < $entries['count']; $i++) {
                    $return[$i]['nni'] = $entries[$i]['uid'][0];

                    if (isset($entries[$i]['sn']['count']) && $entries[$i]['sn']['count'] >= 1) {
                        $mot = str_replace('-', '¿ ', $entries[$i]['sn'][0]);
                        $mot = ucwords(strtolower($mot));
                        $mot = str_replace('¿ ', '-', $mot);
                        $return[$i]['nom'] = $mot;
                    } else {
                        $return[$i]['nom'] = "";
                    ...
                    if (isset($entries[$i]['givenname']['count']) && $entries[$i]['givenname']['count'] >= 1) {
                        $mot = str_replace('-', '¿ ', $entries[$i]['givenname'][0]);
                        $mot = ucwords(strtolower($mot));
                        $mot = str_replace('¿ ', '-', $mot);
                        $return[$i]['prenom'] = $mot;
                    } else {
                    ...
                    if (isset($entries[$i]['mail']['count']) && $entries[$i]['mail']['count'] >= 1) {
                        $return[$i]['mail'] = strtolower($entries[$i]['mail'][0]);
                    } else {
                        $return[$i]['mail'] = "";
                    }

                    if (isset($entries[$i]['departmentnumber']['count']) && $entries[$i]['departmentnumber']['count'] >= 1) {
                        $return[$i]['departmentnumber'] = strtolower($entries[$i]['departmentnumber'][0]);
                    } else {
                        $return[$i]['departmentnumber'] = "";
                    }

                    if (isset($entries[$i]['gardianhierarchie']['count']) && $entries[$i]['gardianhierarchie']['count'] >= 1) {
                        $return[$i]['gardianhierarchie'] = strtolower($entries[$i]['gardianhierarchie'][0]);
                    } else {
                        $return[$i]['gardianhierarchie'] = "";
                    }
                }
     ...
     */
    private function search_ad($account, $req_attrs = array('cn', 'dn', 'uid', 'mail', 'departmentNumber', 'gardianHierarchie'))
    {
        // preset the result
        $result = array();

        // set up the search parameters
        $filter = '(&(uid=' . $this->ldap_escape($account, false) . ')(!(departmentNumber=PARTIS)))';
     ...
     * @return bool or array
     */
    private function search_multiple_ad($account, $req_attrs = array('cn', 'dn', 'uid', 'mail', 'departmentNumber', 'gardianHierarchie'))
    {
        // preset the result
        $result = array();

        // set up the search parameters
        // ajout de l'* pour (cn=Mike*) for any cn starting with Mike. cn=*Mike* will get you anything containing Mike.
        $filter = '(&(uid='.$this->ldap_escape($account, false).'*)(!(departmentNumber=PARTIS)))';
        if (strlen($this->_start_ou) > 0) {
            $search_dn = $this->_start_ou . ',' . $this->_base_dn;
        } else {
        ...
        if ($search = ldap_search($this->_ldap_conn, $search_dn, $filter, $req_attrs)) {
            ...
            $result = ldap_get_entries($this->_ldap_conn, $search);
